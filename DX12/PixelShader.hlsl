...
float4 main(VertexToPixel input) : SV_TARGET
{
	...
	const float roughness = RoughnessMap.Sample(BasicSampler, input.uv).r;
	const float metal = MetalMap.Sample(BasicSampler, input.uv).r;
	
	// Specular color - Assuming albedo texture is actually holding specular color if metal == 1
	// Note the use of lerp here - metal is generally 0 or 1, but might be in between
	// because of linear texture sampling, so we want lerp the specular color to match
	const float3 specColor = lerp(F0_NON_METAL.rrr, surfaceColor.rgb, metal);
	...
	//return float4(pow(surfaceColor.rgb, 1.0f / 2.2f), 1.0f);
